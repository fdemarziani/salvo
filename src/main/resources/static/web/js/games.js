...
$('#login-form').on('submit', function (event) {
    ...
    if (submitButton == "login") {
        ...
        $.post("/api/players",
            { userName: $("#username").val(), //userName blanco
                password: $("#password").val() })
            .done(function(data) {
        ...
        for (let i = 0; i < gamesData.length; i++) {
            ...
            for (let j = 0; j < gamesData[i].gamePlayers.length; j++) {
                ...
                if (gamesData[i].gamePlayers.length == 1 && data.player.id != null && data.player.id != gamesData[i].gamePlayers[j].player.id) {
                    $('<td>' + gamesData[i].gamePlayers[0].player.userName /*email*/ + '</td><td class="yellow500">WAITING FOR PLAYER</td>').appendTo(row);
                ...
                if (gamesData[i].gamePlayers[j].player.id == data.player.id) {
                    gpid = gamesData[i].gamePlayers[j].gpid; //gpid <- id
                    isLoggedPlayer = true;
                }
            }
            ...
            for (let j = 0; j < gamesData[i].gamePlayers.length; j++) {
                if (!playersIds.includes(gamesData[i].gamePlayers[j].player.id)) {
                    ...
                    let playerScoreData = {
                        ...
                        "userName": gamesData[i].gamePlayers[j].player.userName, //email
                        "scores": [],
                        "Total": 0.0 //total
                    };
...
function addScoresToPlayersArray(playersArray) {
        for (let i = 0; i < gamesData.length; i++) {
            ...
            for (let j = 0; j < gamesData[i].scores.length; j++) {
                ...
                let scorePlayerId = gamesData[i].scores[j].player; //playerID
                ...
                for (let k = 0; k < playersArray.length; k++) {
                    ...
                    if (playersArray[k].id == scorePlayerId) {
                        if(gamesData[i].scores[j].score != null){
                            ...
                            playersArray[k].Total += gamesData[i].scores[j].score; //total
                        }
                    }
                }
            }
...
function showScoreBoard(playersArray) {
        ...
        for (let m = 0; m < playersArray.length; m++) {
            ...
            if (playersArray[m].scores.length > 0) {
                ...
                $('<td>' + playersArray[m].userName + '</td>').appendTo(row); //email
